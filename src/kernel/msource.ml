...
let find_line line {text} =
  ...
  if line = 1 then 0 else
    ...
    try
      ...
      log "find_line" "line %d out of bounds (max = %d)" line (line - !line');
      String.length text
    with Found n ->
      n + 1
  ...
  if col = 0 then offset else
    try
      ...
      if (offset + col) > (String.length text) then begin
        log "find_offset" "%d:%d out of file bounds" line col
      end;
      offset + col
    with Found off -> off
...
let get_logical {text} = function
  ...
  | `Offset _ | `End as r ->
    ...
    let offset = match r with
      | `Offset x when x > len ->
        log "get_logical" "offset %d out of bounds (size is %d)" x len;
        len
      | `Offset x ->
        assert (x >= 0);
        x
